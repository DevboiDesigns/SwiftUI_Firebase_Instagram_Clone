...
class CommentViewModel: ObservableObject {
    ...
    func uploadComment(commentText: String) {
        ...
        COLLECTION_POSTS.document(postId) .collection("post-comments").addDocument(data: data) { error in
            if let error = error {
                print("DEBUG: Error uploading commetn \(error.localizedDescription)")
                return
            }
        }
    ...
    func fetchComments() {
        ...
        let query = COLLECTION_POSTS.document(postId).collection("post-comments").order(by: "timestamp", descending: true) // ORDERS by Timestamp Descending
